...
```
libs/
├── libbgpstream-2.2.0/           # BGPstream（含构建产物）
│   ├── lib/          # libbgpstream 核心: MRT 读取、过滤、流式处理
│   ├── tools/        # CLI: bgpstream, bgpstream-json 等
│   ├── common/       # 跨平台公共函数
│   ├── test/         # 单元测试和集成测试
│   ├── docs/         # 文档
│   ├── m4/           # Autoconf 宏
│   ├── configure / configure.ac / Makefile.am / Makefile.in
│   ├── Makefile / config.h / config.log / config.status  # 构建产物
│   └── AUTHORS / LICENSE / ChangeLog / README
│
├── wandio-4.2.4-1/               # wandio（含构建产物）
│   ├── lib/          # libwandio 核心: 透明压缩读写
│   ├── tools/        # wandio CLI 转换工具
│   ├── test/         # 测试
│   ├── m4/           # Autoconf 宏
│   ├── debian/       # Debian 打包脚本
│   ├── rpm/          # RPM 打包脚本
│   ├── autom4te.cache/  # Autoconf 缓存
│   ├── configure / configure.ac / Makefile.am / Makefile.in
│   ├── Makefile / config.h / config.log / config.status
│   └── COPYING / AUTHORS / README
│
├── libbgpstream-2.2.0.tar.gz     # 原始源码包
└── 4.2.4-1.tar.gz                # wandio 原始源码包
```

---

## libbgpstream-2.2.0 —— BGP 监测框架
...
- **数据源抽象：** 一套 API 同时支持 Route Views、RIPE RIS、BGPmon、OpenBMP
- **实时和历史：** `bgpstream_create()` 可选择实时流或历史回放模式
- **记录过滤：** 按 IP 前缀、ASN、对等体 ASN、采集器名称、记录类型过滤
- **MRT 格式：** 读取标准 MRT TABLE_DUMP_V2 和 UPDATE 消息
- **内存管理：** 内置 `bgpstream_record_*` API 管理记录生命周期

### 在流水线中的角色

```
[Route Views / RIPE RIS 服务器]
         │
         ▼
  libbgpstream ← 通过 bgpstream API 获取实时/历史数据
         │
         ▼
  MRT .bz2 文件 ← 保存到本地磁盘
         │
  ...
  wandio ← 透明解压 .bz2
         │
         ▼
  bgpdump (在 ../mrtdb/libbgpdump-1.6.0/) ← 解析 MRT 二进制
         │
         ▼
  cmrtdb / mrtdb_user ← 索引和查询
```

## wandio-4.2.4-1 —— I/O 抽象库
...
- **格式透明：** `wandio_create()` 自动检测 magic 字节，选择正确的解压器
- **多格式支持：** raw、gzip、bzip2、LZO、LZMA (xz)、ZSTD
- **线程安全：** 支持多线程并发读取
- **零拷贝：** peak/write 接口减少内存复制
- **高吞吐：** 专为大规模网络数据处理优化（数百 MB/s 级）
...
# 生成 configure（如果修改了 configure.ac）
./bootstrap.sh
...
| 问题 | 原因 | 解决 |
|------|------|------|
| `configure: error: wandio not found` | wandio 未安装或路径不对 | `sudo make install` 确保 libwandio 在 `/usr/local/lib` |
| `libcurl not found` | libcurl 开发包缺失 | `sudo apt install libcurl4-openssl-dev` |
| `lzma.h: No such file` | xz 开发包缺失 | `sudo apt install liblzma-dev` |
| `zstd.h: No such file` | ZSTD 开发包缺失 | `sudo apt install libzstd-dev` |
| `bzlib.h: No such file` | bzip2 开发包缺失 | `sudo apt install libbz2-dev` |

---

## 原始源码包

- `libbgpstream-2.2.0.tar.gz` —— BGPstream 2.2.0 官方发布包
- `4.2.4-1.tar.gz` —— wandio 4.2.4 官方发布包
...
- **mrtdb** (`../mrtdb/`) —— 主要消费者，使用这些库进行 BGP 数据处理
- **mrtdb/libbgpdump-1.6.0/** —— 另一个 MRT 格式解析库（互补关系：bgpdump 做解析，bgpstream 做数据采集）
- **mrtdb/mrt_data/** —— MRT 转储文件
