...
int process_mrtd_table_dump_v2_ipv4_unicast(struct mstream *s, BGPDUMP_ENTRY *entry){
    ...
    if (level >= 240)
    {
	    ...
	    mstream_get(s, &prefixdata->prefix.v6_addr.s6_addr[0], (prefixdata->prefix_length + 7)/8);
        prefixdata->prefix.v6_addr.s6_addr[11] = level;
        prefixdata->prefix.v6_addr.s6_addr[15] = 0xff;
    }
    else
    {
        prefixdata->prefix_length = level;
        bzero(&prefixdata->prefix.v6_addr.s6_addr, 16);
	    mstream_get(s, &prefixdata->prefix.v6_addr.s6_addr[0], (prefixdata->prefix_length+7)/8);
        prefixdata->prefix.v6_addr.s6_addr[15] = 0xff;
    }
