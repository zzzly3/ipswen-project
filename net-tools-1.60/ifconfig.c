...
static int set_ifstate(char *parent, struct ipswen_addr *addr,
                       struct ipswen_addr *nm, struct ipswen_addr *bc,
		       int flag);
#else
...
#if HAVE_IPSWEN
		{ /* add an IPswen address "x:i.y:j.z:k(level)a.b.c.d" */
			//puts("add - IPswen address x:i.y:j.z:k(level)a.b.c.d");
			struct ipswen_addr ip, nm, bc;
			safe_strncpy(host, *spp, (sizeof host));
...
#if HAVE_IPSWEN
		{ /* delete an IPswen address "x:i.y:j.z:k(level)a.b.c.d" */
			//puts("del - IPswen address x:i.y:j.z:k(level)a.b.c.d");
			struct ipswen_addr ip, nm, bc;
			safe_strncpy(host, *spp, (sizeof host));
			if (inet_aftype.input(0, host, &_sa) < 0) {
			    ap->herror(host);
...
#if HAVE_IPSWEN
    if (memcmp(&sin->swen_addr, &ptr->swen_addr, sizeof(struct ipswen_addr))) {
	return 0;
    }
#else
                           ...
                           struct ipswen_addr *bc)
{
    ...
    memcpy(nm, &sin->swen_addr, sizeof(*nm));
    sin = (struct sockaddr_in *)&i->broadaddr_sas;
    memcpy(bc, &sin->swen_addr, sizeof(*bc));
    return 0;
}

static int set_ip_using(const char *name, int c,
		       ...
		       int flag)
{
    ...
    i = for_all_interfaces((int (*)(struct interface *,void *))do_ifcmd, &pt);
    if (i == -1)
	return -1;
    if (i == 1)
	return 0;
	...
	return -1;
    if (set_ip_using(buf, SIOCSIFADDR, addr) == -1)
	return -1;
