...
#if HAVE_IPSWEN
    /* Default is special, meaning 0.0.0.0(0) */
    if (!strcmp(name, "default")) {
	sin->swen_addr.base = INADDR_ANY;
	sin->swen_addr.ext.level = 0;
	return (1);
    }
    /* Look to see if it's a dotted decimal */
			 ...
			 int numeric, unsigned int netmask)
{
    ...
    if (swen_addr->base == INADDR_ANY) {
	    ...
	    // for 0.0.0.0/1 we skip getnetbyname()
            safe_strncpy(name, "0.0.0.0(0)", len);
            return (0);
	}
    }

    // it is a host address if flagged or any host bits set
    if ((swen_addr->base & (~netmask)) != 0 || (numeric & 0x4000))
	host = 1;
    pn = INET_nn;
    while (pn != NULL) {
	if (pn->addr.swen_addr.base == swen_addr->base && pn->host == host) {
...
#endif
	ent = gethostbyaddr((char *) &swen_addr->base, 4, AF_INET);
	if (ent != NULL)
    ...
    if ((ent == NULL) && (np == NULL)){
    	if(ipswen_ntoa(&sin->sin_addr, buf, IPSWEN_ADDRSTRLEN))
