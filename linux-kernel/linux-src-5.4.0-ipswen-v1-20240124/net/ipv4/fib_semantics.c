...
void rtmsg_fib(int event, __be32 key, struct fib_alias *fa, int dst_len,
	       u32 tb_id, const struct nl_info *info, unsigned int nlm_flags, u64 dst_ext)
#else
void rtmsg_fib(int event, __be32 key, struct fib_alias *fa, int dst_len,
...
#if IS_ENABLED(CONFIG_IP_SWEN)
	err = fib_dump_info(skb, info->portid, seq, event, tb_id, fa->fa_type,
			    key, dst_len, fa->fa_tos, fa->fa_info, nlm_flags, dst_ext);
#else
	err = fib_dump_info(skb, info->portid, seq, event, tb_id, fa->fa_type,
...
{
#if IS_ENABLED(CONFIG_IP_SWEN)
	if (cfg->fc_type != RTN_LOCAL || !cfg->fc_dst.base ||
	    fib_prefsrc != cfg->fc_dst.base) {
#else
...
static u64 Key_ext2u64(const struct ipswen_addr *key)
{
/*
	u64 x = 0;
	int i = 0;
...
#if IS_ENABLED(CONFIG_IP_SWEN)
		{
			u64 x = Key_ext2u64(&nhc->nhc_gw.ips);
			nla_put(skb, RTA_GATEWAY_EXT, sizeof(u64), &x);
		}
...
int fib_dump_info(struct sk_buff *skb, u32 portid, u32 seq, int event,
		  ...
		  struct fib_info *fi, unsigned int flags, u64 dst_ext)
#else
int fib_dump_info(struct sk_buff *skb, u32 portid, u32 seq, int event,
		  u32 tb_id, u8 type, __be32 dst, int dst_len, u8 tos,
...
#endif
{
	...
	if (rtm->rtm_dst_len &&
		...
		goto nla_put_failure;
#if IS_ENABLED(CONFIG_IP_SWEN)
	nla_put(skb, RTA_DST_EXT, sizeof(u64), &dst_ext);
#endif
	if (fi->fib_priority &&
