...
static void inet_hash_insert(struct net *net, struct in_ifaddr *ifa)
{
	...
	u32 hash = inet_addr_hash(net, ifa->ifa_local.base + ifa->ifa_local.ext.level);
#else
			   ...
			   int destroy, struct nlmsghdr *nlh, u32 portid)
{
	...
	if (!(ifa1->ifa_flags & IFA_F_SECONDARY)) {
		...
		while ((ifa = rtnl_dereference(*ifap1)) != NULL) {
			...
			if (!(ifa->ifa_flags & IFA_F_SECONDARY) ||
#if IS_ENABLED(CONFIG_IP_SWEN)
			    !ipswen_is_eq(&ifa1->ifa_mask, &ifa->ifa_mask) ||
			    !inet_ifa_match(ifa1->ifa_address.base, ifa)) {
#else
...
#if IS_ENABLED(CONFIG_IP_SWEN)
		if (ipswen_is_eq(&ifa1->ifa_mask, &ifa->ifa_mask) &&
		    inet_ifa_match(ifa1->ifa_address.base, ifa))
#else
			     ...
			     u32 portid, struct netlink_ext_ack *extack)
{
...
#if IS_ENABLED(CONFIG_IP_SWEN)
	if (!ifa->ifa_local.base && !ifa->ifa_local.ext.level) {
#else
	if (!ifa->ifa_local) {
#endif
		inet_free_ifa(ifa);
	...
	while (ifa1) {
		...
		if (ipswen_is_eq(&ifa1->ifa_mask, &ifa->ifa_mask) &&
		    inet_ifa_match(ifa1->ifa_address.base, ifa)) {
			if (ipswen_is_eq(&ifa1->ifa_local, &ifa->ifa_local)) {
#else
		if (ifa1->ifa_mask == ifa->ifa_mask &&
		    inet_ifa_match(ifa1->ifa_address, ifa)) {
	 ...
	 */
#if IS_ENABLED(CONFIG_IP_SWEN)
	ivi.ivi_addr = ifa->ifa_address.base;
#else
	...
	if (!(ifa->ifa_flags & IFA_F_SECONDARY)) {
#if IS_ENABLED(CONFIG_IP_SWEN)
		prandom_seed((__force u32) ifa->ifa_local.base
				+ ifa->ifa_local.ext.level);
#else
...
#if IS_ENABLED(CONFIG_IP_SWEN)
	if (ipswen_is_loopback(&ifa->ifa_local))
#else
...
#if IS_ENABLED(CONFIG_IP_SWEN)
		if (ifa->ifa_mask.base == mask && inet_ifa_match(prefix, ifa))
#else
...
#if IS_ENABLED(CONFIG_IP_SWEN)
		.imr_multiaddr.s_addr = ifa->ifa_address.base,
#else
			    ...
			    struct netlink_ext_ack *extack)
{
	...
	for (ifap = &in_dev->ifa_list; (ifa = rtnl_dereference(*ifap)) != NULL;
		...
		if (tb[IFA_LOCAL] &&
#if IS_ENABLED(CONFIG_IP_SWEN)
		    ifa->ifa_local.base != nla_get_in_addr(tb[IFA_LOCAL]))
#else
...
#if IS_ENABLED(CONFIG_IP_SWEN)
		if (ipswen_is_mcast(&ifa->ifa_address))
#else
...
#if IS_ENABLED(CONFIG_IP_SWEN)
	ifa->ifa_mask.base = inet_make_mask(ifm->ifa_prefixlen);
#else
...
#if IS_ENABLED(CONFIG_IP_SWEN)
	ifa->ifa_local.base = nla_get_in_addr(tb[IFA_LOCAL]);
	ifa->ifa_address.base = nla_get_in_addr(tb[IFA_ADDRESS]);

	if (tb[IFA_BROADCAST])
		ifa->ifa_broadcast.base = nla_get_in_addr(tb[IFA_BROADCAST]);
#else
	ifa->ifa_local = nla_get_in_addr(tb[IFA_LOCAL]);
...
static struct in_ifaddr *find_matching_ifa(struct in_ifaddr *ifa)
{
	...
	in_dev_for_each_ifa_rtnl(ifa1, in_dev) {
		...
		if ( //ifa1->ifa_mask == ifa->ifa_mask &&
			ipswen_is_eq(&ifa1->ifa_mask, &ifa->ifa_mask) &&
			inet_ifa_match(ifa1->ifa_address.base, ifa) &&
			ipswen_is_eq(&ifa1->ifa_local, &ifa->ifa_local))
#else
		if (ifa1->ifa_mask == ifa->ifa_mask &&
		    inet_ifa_match(ifa1->ifa_address, ifa) &&
...
int devinet_ioctl(struct net *net, unsigned int cmd, struct ifreq *ifr)
{
...
#if IS_ENABLED(CONFIG_IP_SWEN)
				    ipswen_is_eq(&sin_orig.swen_addr,
							&ifa->ifa_local)) {
#else
	...
	case SIOCGIFBRDADDR:	/* Get the broadcast address */
		ret = 0;
#if IS_ENABLED(CONFIG_IP_SWEN)
		sin->swen_addr = ifa->ifa_broadcast;
#else
	...
	case SIOCSIFADDR:	/* Set interface address (and family) */
		...
		if (!ifa) {
			...
			ret = 0;
#if IS_ENABLED(CONFIG_IP_SWEN)
			if (ipswen_is_eq(&ifa->ifa_local, &sin->swen_addr))
				break;
			inet_del_ifa(in_dev, ifap, 0);
			ipswen_addr_bzero(&ifa->ifa_broadcast);
#else
...
#if IS_ENABLED(CONFIG_IP_SWEN)
		ifa->ifa_address = ifa->ifa_local = sin->swen_addr;

		if (!(dev->flags & IFF_POINTOPOINT)) {
			ifa->ifa_prefixlen = inet_abc_len(ifa->ifa_address.base);
			ipswen_addr_mask(&ifa->ifa_mask, 0, ifa->ifa_prefixlen);
			if ((dev->flags & IFF_BROADCAST) &&
			    ifa->ifa_prefixlen < 31)
				ifa->ifa_broadcast.base =
					ifa->ifa_address.base | ~ifa->ifa_mask.base;
		}
		else {
			ifa->ifa_prefixlen = 32;
			ifa->ifa_mask.base = inet_make_mask(32);
			ifa->ifa_mask.ext.level = 0;
		}
#else
	...
	case SIOCSIFBRDADDR:	/* Set the broadcast address */
		ret = 0;
#if IS_ENABLED(CONFIG_IP_SWEN)
		if (!ipswen_is_eq(&ifa->ifa_broadcast, &sin->swen_addr)) {
			inet_del_ifa(in_dev, ifap, 0);
			ifa->ifa_broadcast = sin->swen_addr;
			inet_insert_ifa(ifa);
		}
#else
	...
	case SIOCSIFDSTADDR:	/* Set the destination address */
		ret = 0;
#if IS_ENABLED(CONFIG_IP_SWEN)
		if (ipswen_is_eq(&ifa->ifa_address, &sin->swen_addr))
			break;
		ret = -EINVAL;
		if (inet_abc_len(sin->sin_addr.s_addr) < 0)
			break;
	...
	case SIOCSIFNETMASK: 	/* Set the netmask for the interface */
		...
		ret = 0;
#if IS_ENABLED(CONFIG_IP_SWEN)
		if (ifa->ifa_mask.base != sin->swen_addr.base) {
			__be32 old_mask = ifa->ifa_mask.base;
			inet_del_ifa(in_dev, ifap, 0);
			ifa->ifa_mask = sin->swen_addr;
			ifa->ifa_prefixlen = ipswen_addr_masklen(&ifa->ifa_mask);
			...
			if ((dev->flags & IFF_BROADCAST) &&
			    (ifa->ifa_prefixlen < 31) &&
			    (ifa->ifa_broadcast.base ==
			     (ifa->ifa_local.base | ~old_mask))) {
				ifa->ifa_broadcast.base =
					(ifa->ifa_local.base | ~sin->sin_addr.s_addr);
			}
			inet_insert_ifa(ifa);
		}
#else
...
#if IS_ENABLED(CONFIG_IP_SWEN)
		(*(struct sockaddr_in *)&ifr.ifr_addr).swen_addr =
								ifa->ifa_local;
#else
...
#if IS_ENABLED(CONFIG_IP_SWEN)
		if (!addr && (local == ifa->ifa_local.base || !local) &&
		    min_scope <= scope) {
			addr = ifa->ifa_local.base;
			if (same)
				break;
...

{
	...
	in_dev_for_each_ifa_rtnl(ifa, in_dev) {
		...
		arp_send(ARPOP_REQUEST, ETH_P_ARP,
			 ifa->ifa_local.base, dev,
			 ifa->ifa_local.base, NULL,
			 dev->dev_addr, NULL);
#else
		arp_send(ARPOP_REQUEST, ETH_P_ARP,
			 ifa->ifa_local, dev,
	...
	case NETDEV_UP:
		...
		if (dev->flags & IFF_LOOPBACK) {
			...
			if (ifa) {
				INIT_HLIST_NODE(&ifa->hash);
#if IS_ENABLED(CONFIG_IP_SWEN)
				ifa->ifa_local.base = ifa->ifa_address.base =
					htonl(INADDR_LOOPBACK);
				ifa->ifa_prefixlen = 8;
				ifa->ifa_mask.base = inet_make_mask(8);
#else
				ifa->ifa_local =
...
#if IS_ENABLED(CONFIG_IP_SWEN)
	if ((ifa->ifa_address.base &&
	     nla_put_in_addr(skb, IFA_ADDRESS, ifa->ifa_address.base)) ||
	    (ifa->ifa_local.base &&
	     nla_put_in_addr(skb, IFA_LOCAL, ifa->ifa_local.base)) ||
	    (ifa->ifa_broadcast.base &&
	     nla_put_in_addr(skb, IFA_BROADCAST, ifa->ifa_broadcast.base)) ||
	    (ifa->ifa_label[0] &&
