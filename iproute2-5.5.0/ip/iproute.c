...
#ifdef HAVE_IPSWEN
	if (tb[RTA_DST]) {
		memcpy(&dst.data, RTA_DATA(tb[RTA_DST]), 4);
		if (tb[RTA_DST_EXT])
			memcpy(dst.data + 1, RTA_DATA(tb[RTA_DST_EXT]), 8);
	}
#else
	if (tb[RTA_DST])
...
#ifdef HAVE_IPSWEN
		if (tb[RTA_GATEWAY]) {
			memcpy(&via.data, RTA_DATA(tb[RTA_GATEWAY]), 4);
			if (tb[RTA_GATEWAY_EXT])
				memcpy(via.data + 1, RTA_DATA(tb[RTA_GATEWAY_EXT]), 8);
		}
#else
		if (tb[RTA_GATEWAY])
...
static void print_rta_gateway_1(FILE *fp, unsigned char family, const struct rtattr *rta, const struct rtattr *rtb)
{
	struct ipswen_addr ips;
	char gateway[64];
	memcpy(&ips, RTA_DATA(rta), 4);
...
#ifdef HAVE_IPSWEN
			if (tb[RTA_GATEWAY])
				print_rta_gateway_1(fp, r->rtm_family,
						  tb[RTA_GATEWAY], tb[RTA_GATEWAY_EXT]);
#else
			if (tb[RTA_GATEWAY])
...
int print_route(struct nlmsghdr *n, void *arg)
{
	...
	if (tb[RTA_DST]) {
		...
		if (r->rtm_dst_len != host_len) {
			...
			if (tb[RTA_DST_EXT]) {
				struct ipswen_addr ips;
				char tmp[64];
				memcpy(&ips, RTA_DATA(tb[RTA_DST]), 4);
				ips.ext = *(struct ipswen_ext *)RTA_DATA(tb[RTA_DST_EXT]);
				ipswen_ntoa(&ips, tmp, sizeof(tmp));
	...
	if (tb[RTA_GATEWAY] && filter.rvia.bitlen != host_len)
		print_rta_gateway_1(fp, r->rtm_family, tb[RTA_GATEWAY], tb[RTA_GATEWAY_EXT]);
#else
	if (tb[RTA_GATEWAY] && filter.rvia.bitlen != host_len)
		...
		if (strcmp(*argv, "via") == 0) {
			...
			if (addr.family == r->rtm_family) {
				if (rta_addattr_l(rta, len, RTA_GATEWAY,
					...
					return -1;
#ifdef HAVE_IPSWEN
				if (rta_addattr_l(rta, len, RTA_GATEWAY_EXT,
						  addr.data + 1, 8))
					return -1;
#endif
		...
		if (strcmp(*argv, "src") == 0) {
			...
			if (addr.family == req.r.rtm_family) {
				addattr_l(&req.n, sizeof(req), RTA_GATEWAY,
					  &addr.data, addr.bytelen);
				addattr_l(&req.n, sizeof(req), RTA_GATEWAY_EXT,
					  addr.data + 1, 8);
			}
			else
