...
static struct nl_want_attrs ifa_attr_want4[BIRD_IFA_MAX] = {
  [IFA_ADDRESS]	  = { 1, 1, sizeof(u32) },
  [IFA_LOCAL]	  = { 1, 1, sizeof(u32) },
  [IFA_BROADCAST] = { 1, 1, sizeof(u32) },
...
static inline u64 rta_get_u64(struct rtattr *a)
{ return *(u64 *) RTA_DATA(a); }

static inline u64 rta_get_ips_ext(struct rtattr *a)
{
	return rta_get_u64(a);
}
...
{ return ip4_ntoh(_MISWEN_D(*(u32 *)RTA_DATA(a), 0, 0)); }
...
static inline ip_addr rta_get_via(struct rtattr *a)
{
  ...
  switch(v->rtvia_family) {
    case AF_INET:  return ipa_from_ip4(ip4_ntoh(_MISWEN_D(*(u32 *) v->rtvia_addr, 0, 0)));
...
static inline void
nl_add_attr_u64(struct nlmsghdr *h, uint bufsize, int code, u64 data)
{
	nl_add_attr(h, bufsize, code, &data, 8);
}

static inline void
nl_add_attr_ips_ext(struct nlmsghdr *h, uint bufsize, int code, ip_addr ipa)
{
	ip4_addr ips = ipa_to_ip4(ipa);
  u64 x = *(u64 *)&ips.addr.ext;
	nl_add_attr_u64(h, bufsize, code, x);
}
...
{
  ip4 = ip4_hton(ip4);
  nl_add_attr(h, bufsize, code, &ip4, sizeof(u32));
}
...
#ifdef HAVE_IPSWEN
      nl_add_attr_ips_ext(h, bufsize, RTA_GATEWAY_EXT, nh->gw);
#endif
    }
    else
...
#ifdef HAVE_IPSWEN
    nl_add_attr_ips_ext(&r->h, rsize, RTA_DST_EXT, net_prefix(net->n.addr));
...
nl_parse_route(struct nl_parse_state *s, struct nlmsghdr *h)
{
  ...
  switch (i->rtm_family)
	...
	net_fill_ip4(&dst, IP4_NONE, 0);
      ...
      if (a[RTA_DST_EXT])
      {
  u64 x = rta_get_ips_ext(a[RTA_DST_EXT]);
  ((net_addr_ip4 *)&dst)->prefix.addr.ext = *(struct ipswen_ext *)&x;
      }
  ...
  log(L_DEBUG "KRT: Received route %N", &dst);
...
#ifdef HAVE_IPSWEN
      if (a[RTA_GATEWAY_EXT])
      {
	ip4_addr tmp = ipa_to_ip4(ra->nh.gw);
  u64 x = rta_get_ips_ext(a[RTA_GATEWAY_EXT]);
	tmp.addr.ext = *(struct ipswen_ext *)&x;
	ra->nh.gw = ipa_from_ip4(tmp);
      }
#endif
