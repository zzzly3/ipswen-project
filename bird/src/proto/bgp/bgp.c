...
bgp_conn_enter_established_state(struct bgp_conn *conn)
{
  ...
  /* 
    Whether the ipswen extension is enabled. 
    It doesn't mean the ipswen MP channel is available, which additionally requires an available ip4 channel.
  */
#ifdef HAVE_IPSWEN
  const struct bgp_af_caps *ipswen_loc = bgp_find_af_caps(local, BGP_AF_IPSWEN);
  const struct bgp_af_caps *ipswen_rem = bgp_find_af_caps(peer, BGP_AF_IPSWEN);
  p->ipswen_extension = ipswen_loc && ipswen_rem && ipswen_loc->ready && ipswen_rem->ready;
#endif
...
bgp_channel_start(struct channel *C)
{
  ...
  if (ipa_zero(c->next_hop_addr))
  {
    ...
    #ifdef HAVE_IPSWEN
    /* Don't use ipswen address in non-ipswen connection */
    if (ipa_is_ipswen(c->next_hop_addr) && !p->ipswen_extension)
