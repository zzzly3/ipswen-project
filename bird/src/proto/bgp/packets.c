...
bgp_prepare_capabilities(struct bgp_conn *conn)
{
  ...
  {
  #ifdef HAVE_IPSWEN
    if (c->afi == BGP_AF_IPV4)
    {
      ac = &caps->af_data[caps->af_count++];
      ac->afi = BGP_AF_IPSWEN;  /* FIXME: control the on-off in configuration */
      ac->ready = 1;
    }
  #endif
    ac = &caps->af_data[caps->af_count++];
...
bgp_use_next_hop(struct bgp_export_state *s, eattr *a)
{
  ...
  #ifdef HAVE_IPSWEN
  if (ipa_is_ipswen(*nh) && !s->proto->ipswen_extension)
    return 0;
  #endif
...
bgp_use_gateway(struct bgp_export_state *s)
{
  ...
  #ifdef HAVE_IPSWEN
  if (ipa_is_ipswen(ra->nh.gw) && !s->proto->ipswen_extension)
    return 0;
  #endif
...
bgp_update_next_hop_ip(struct bgp_export_state *s, eattr *a, ea_list **to)
{
  ...
  #ifdef HAVE_IPSWEN
  if (ipa_is_ipswen(nh[0]) && !s->proto->ipswen_extension)
    REJECT(BAD_NEXT_HOP MISMATCHED_AF, nh[0], s->channel->desc->name);
  #endif
...
bgp_encode_next_hop_ip(struct bgp_write_state *s, eattr *a, byte *buf, uint size UNUSED)
{
  ...
  {
    ip4_addr addr = ipa_to_ip4(nh[0]);
  #ifdef HAVE_IPSWEN
    if (s->proto->ipswen_extension)
    {
      return put_ipswen(buf, addr) - (void*)buf;
    }
  #endif
    put_ip4(buf, addr);
    return 4;
...
#ifdef HAVE_IPSWEN
  else if (len >= 5 && len <= 12 && s->proto->ipswen_extension)
  {
    ip4_addr addr;
    if (!get_ipswen(data, &addr, len))
      bgp_parse_error(s, 9);
    nh[0] = ipa_from_ip4(addr);
    nh[1] = IPA_NONE;
  }
#endif
  else if (len == 32)
...
bgp_encode_nlri_ip4(struct bgp_write_state *s, struct bgp_bucket *buck, byte *buf, uint size)
{
  ...
  while (!EMPTY_LIST(buck->prefixes) && (size >= BGP_NLRI_MAX))
  {
    ...
    #ifdef HAVE_IPSWEN
    if (_LEVEL(net->prefix) != 0)
    {
      log(L_ERR "%s: ipswen prefix %N in ip4-only NLRI", s->proto->p.name, net);
      return 0;
    }
    #endif
...
static uint
bgp_encode_nlri_ipswen(struct bgp_write_state *s, struct bgp_bucket *buck, byte *buf, uint size)
{
  if (!s->proto->ipswen_extension)
  {
    log(L_ERR "%s: ipswen prefix in ip4-only NLRI", s->proto->p.name);
    return 0;
  }
  ...
  while (!EMPTY_LIST(buck->prefixes) && (size >= BGP_NLRI_MAX))
  {
    ...
    *pos = net->pxlen + 8;
    ...
    /* Encode MPLS labels (unsupported in ipswen at present) */
    // if (s->mpls)
    //   bgp_encode_mpls_labels(s, s->mpls_labels, &pos, &size, pos - 1);
...
static void
bgp_decode_nlri_ipswen(struct bgp_parse_state *s, byte *pos, uint len, rta *a)
{
  if (!s->proto->ipswen_extension)
    bgp_parse_error(s, 10);
  ...
  while (len)
  {
    ...
    ADVANCE(pos, len, 1);
    l -= 8;
    if (abs(level) > 7 || l > IP4_MAX_PREFIX_LENGTH + (unsigned)abs(level) * 8)
      bgp_parse_error(s, 10);
...
{
  #ifdef HAVE_IPSWEN
  return (s->channel->afi == BGP_AF_IPV4 && s->proto->ipswen_extension ? bgp_encode_nlri_ipswen : s->channel->desc->encode_nlri)(s, buck, buf, end - buf);
  #else
...
bgp_create_mp_reach(struct bgp_write_state *s, struct bgp_bucket *buck, byte *buf, byte *end)
{
  ...
  #ifdef HAVE_IPSWEN
  put_af3(buf+8, s->channel->afi == BGP_AF_IPV4 && s->proto->ipswen_extension ? BGP_AF_IPSWEN : s->channel->afi);
  #else
...
again: ;
  ...
  struct bgp_write_state s = {
    ...
    .pool = tmp_linpool,
  #ifdef HAVE_IPSWEN
    .mp_reach = (c->afi != BGP_AF_IPV4) || c->ext_next_hop || p->ipswen_extension,
  #else
...
{
#ifdef HAVE_IPSWEN
  int decode_ipswen = s->proto->ipswen_extension && afi == BGP_AF_IPSWEN;
  struct bgp_channel *c = bgp_get_channel(s->proto, decode_ipswen ? BGP_AF_IPV4 : afi);
#else
...
#ifdef HAVE_IPSWEN
  (decode_ipswen ? bgp_decode_nlri_ipswen : c->desc->decode_nlri)(s, nlri, len, a);
#else
