...
mrt_peer_table_entry(struct mrt_table_dump_state *s, u32 peer_id, u32 peer_as, ip_addr peer_ip)
{
  ...
  #ifdef HAVE_IPSWEN
  if (ipa_is_ip4(peer_ip))
    mrt_put_u32(b, ipa_to_ip4(peer_ip).addr.base);
  else
    mrt_put_ip6(b, ipa_to_ip6(peer_ip));
  #else
...
mrt_rib_table_header(struct mrt_table_dump_state *s, net_addr *n)
{
  ...
  if (s->ipv4)
  {
    ...
    #ifdef HAVE_IPSWEN
    mrt_put_u8(b, 0xf0 | (_LEVEL(a) & 0xf));
    #endif
    mrt_put_u8(b, len);
...
mrt_bgp_header(buffer *b, struct mrt_bgp_data *d)
{
  ...
  {
  #ifdef HAVE_IPSWEN
    mrt_put_u32(b, ipa_to_ip4(d->peer_ip).addr.base);
    mrt_put_u32(b, ipa_to_ip4(d->local_ip).addr.base);
  #else
