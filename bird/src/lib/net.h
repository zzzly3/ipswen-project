...
{ char astr[64], bstr[64];
  net_format(a, astr, 64);
  net_format(b, bstr, 64);
  int r = !memcmp(a, b, sizeof(net_addr_ip4));
  // printf("net_equal: %s %s %d\n", astr, bstr, r);
  return r; }
...
{ u32 r = ip4_hash(n->prefix) ^ ((u32) n->pxlen << 26);
  char buf[64];
  net_format((const net_addr *) n, buf, 64);
  // printf("net_hash_ip4: %s %u\n", buf, r);
  return r; }
...
{
  return IPSWEN_VALID_LEVEL(prefix) && (pxlen <= IPSWEN_MAX_PREFIX_LENGTH(prefix)) &&
...
{ ip4_addr prefix = ip4_and(n->prefix, ip4_mkmask(n->pxlen)); _LEVEL(prefix) = _LEVEL(n->prefix); n->prefix = prefix; }
#else
static inline void net_normalize_ip4(net_addr_ip4 *n)
...
{ return (_LEVEL(a) == _LEVEL(prefix) || (_LEVEL(a) < 0 && _LEVEL(prefix) == 0)) && ip4_zero(ip4_and(ip4_xor(a, prefix), ip4_mkmask(pxlen))); }
#else
static inline int ipa_in_px4(ip4_addr a, ip4_addr prefix, uint pxlen)
