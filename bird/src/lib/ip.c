...
{
  int i = 0, j = u32_masklen(a.addr.base), n;
  if (j == 255) return j;
  n = j;
  if (j == 32)
  {
    for (; i < 7; i++)
    {
      j = u32_masklen(a.addr.ext.addr[i]);
      if (j == 255) return j;
      n += j;
      if (j < 8)
      {
  ...
  for (; i < 7; i++)
  {
    if (a.addr.ext.addr[i]) return 255;
  }
  return n;
}
...
#ifdef HAVE_IPSWEN
  if (ipa_is_ip4(*a))
    return ip4_classify(ipa_to_ip4(*a));
#endif
...
{
  ip4_addr na = ip4_hton(a);
  ipswen_ntoa(&na.addr, b, IP4_MAX_TEXT_LENGTH);
  return b + strlen(b);
}
