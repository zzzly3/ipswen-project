...
static __always_inline __u16 csum_replace_u16(__u16 csum, __u16 old, __u16 new)
{
    csum = csum16_add(csum, ~old);
    csum = csum16_add(csum, new);
    return csum;
}

static __always_inline __u16 csum_replace_u32(__u16 csum, __u32 old, __u32 new)
{
    csum = csum16_add(csum, ~(__u16)(old >> 16));
    csum = csum16_add(csum, ~(__u16)old);
    csum = csum16_add(csum, (__u16)(new >> 16));
    csum = csum16_add(csum, (__u16)new);
    return csum;
}
...
int xdp_route_handler(struct xdp_md *ctx) {
    ...
    if (iph->protocol == IPPROTO_ICMP) {
        ...
        if (icmph->type == ICMP_DEST_UNREACH || icmph->type == ICMP_TIME_EXCEEDED ||
            icmph->type == ICMP_PARAMETERPROB || icmph->type == ICMP_REDIRECT ||
            icmph->type == ICMP_SOURCE_QUENCH) {
            struct iphdr *inner_iph = (struct iphdr *)(icmph + 1);
            if (inner_iph + 1 > data_end)
                return XDP_PASS;

            if (redirect_ifindex == LAN_IFINDEX) {
                if (bpf_ntohl(inner_iph->saddr) == OUTER_ADDR) {
                    __u16 inner_csum = ~inner_iph->check;
                    __u16 icmp_csum = ~icmph->checksum;
                    __u16 old_inner_check = inner_iph->check;
                    __u32 old_inner_saddr = inner_iph->saddr;
                    __u32 new_inner_saddr = bpf_htonl((INNER_PREFIX & OPT_ADDR_PREFIX_MASK) |
                                                      (ext_saddr & OPT_ADDR_MASK));

                    inner_csum = csum_replace_u32(inner_csum, old_inner_saddr, new_inner_saddr);
                    icmp_csum = csum_replace_u32(icmp_csum, old_inner_saddr, new_inner_saddr);
                    inner_iph->saddr = new_inner_saddr;
                    inner_iph->check = ~inner_csum;
                    icmp_csum = csum_replace_u16(icmp_csum, old_inner_check, inner_iph->check);
                    icmph->checksum = ~icmp_csum;
                }
                ...
                if ((bpf_ntohl(inner_iph->daddr) & OPT_ADDR_PREFIX_MASK) ==
                    ...
                    __u16 icmp_csum = ~icmph->checksum;
                    __u16 old_inner_check = inner_iph->check;
                    __u32 old_inner_daddr = inner_iph->daddr;
                    __u32 new_inner_daddr = bpf_htonl(OUTER_ADDR);

                    inner_csum = csum_replace_u32(inner_csum, old_inner_daddr, new_inner_daddr);
                    icmp_csum = csum_replace_u32(icmp_csum, old_inner_daddr, new_inner_daddr);
                    inner_iph->daddr = new_inner_daddr;
                    inner_iph->check = ~inner_csum;
                    icmp_csum = csum_replace_u16(icmp_csum, old_inner_check, inner_iph->check);
                    icmph->checksum = ~icmp_csum;
                }
